...
- 12 nt regulatory region:
  `m = 0.8 + 0.4 * GC(regulatory)`
- 11 loci × 24 nt, one locus per trait, annotated
- mutation:
  substitution per base + at most one insertion/deletion event
- fixed-length repair at the 3' end
  (computational simplification, not a biological indel model)
- crossover methods:
